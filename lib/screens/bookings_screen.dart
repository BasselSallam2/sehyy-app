...
import 'package:tabeby_app/screens/doctor_profile_screen.dart'; // (عشان زرار "زيارة")
...
class _BookingsScreenState extends State<BookingsScreen> with TickerProviderStateMixin {
  ...
  void _filterBookings() {
    ...
    final today = DateTime(now.year, now.month, now.day); // (النهاردة الساعة 12ص)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : TabBarView( // 8. الـ TabBarView (المحتوى)
              controller: _tabController,
              children: [
                _buildBookingsList(_todayBookings, canCancel: true),
                _buildBookingsList(_upcomingBookings, canCancel: true),
                _buildBookingsList(_pastBookings, canCancel: false), // (المنتهي مينفعش يتلغي)
              ],
            ),
    );
  }
  ...
  Widget _buildBookingsList(List<dynamic> bookings, {required bool canCancel}) {
    if (bookings.isEmpty) {
      return const Center(child: Text('لا توجد حجوزات في هذا التصنيف', style: TextStyle(color: Colors.grey)));
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // 2. بيانات الموعد
            Text('$dateString - الساعة $timeString', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
